...
class LiturgicalViewModel extends ChangeNotifier {
  LiturgicalData getLiturgicalData(DateTime date) {
    ...
    final formattedDateString = "$currentDayName, ${date.day} Tháng ${date.month}";

    // Mock logic for May 2026 similar to React app
    if (date.year == 2026 && date.month == 5) {
      String week = "VI";
      if (date.day <= 2) week = "IV";
      else if (date.day <= 9) week = "V";
      else if (date.day <= 16) week = "VI";
      else if (date.day <= 23) week = "VII";
      else week = "VIII";

      return LiturgicalData(
        dateString: formattedDateString,
        season: "Tuần $week Phục Sinh",
        feast: date.day == 13 ? "Đức Mẹ Fatima. Lễ Nhớ." : date.day == 14 ? "Thánh Matthias, Tông đồ. Lễ Kính." : "Ngày tuần trong Mùa Phục Sinh",
        readings: [
          LiturgicalReading(
            label: "Bài Đọc I",
            text: date.day == 13 ? "Cv 17, 15.22—18,1" : date.day == 14 ? "Cv 1,15-17.20-26" : "Cv Lời Chúa",
          ...
          LiturgicalReading(
            label: "Đáp Ca",
            text: date.day == 13 ? "Tv 148, 1-2.11-12.13.14" : date.day == 14 ? "Tv 112,1-2.3-4.5-6.7-8" : "Tv Lời Chúa",
          ...
          LiturgicalReading(
            label: "Tin Mừng",
            text: date.day == 13 ? "Ga 16, 12-15" : date.day == 14 ? "Ga 15,9-17" : "Ga Lời Chúa",
